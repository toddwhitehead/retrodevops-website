...
var Module = {
	...
	loadSnapshot: function(fileName, useStorage) {
        ...
        // Load a snapshot image into the emulator
        var request = new XMLHttpRequest();
		request.open('GET', '/snapshots/' + fileName, true);
		...
		request.onload = function(oEvent) {
			if (request.readyState === 4 && request.status === 200) {
				...
				if (arrayBuffer) {
                    ...
                    toastr.success("Snapshot loaded", fileName);
				}
			}
		};
		request.send();
    ...
    loadLocalFile: function (fileName) {
        ...
        // Load a local file into the emulator
        var request = new XMLHttpRequest();
        ...
        request.onload = function (oEvent) {
            if (request.readyState === 4 && request.status === 200) {
                ...
                if (arrayBuffer) {
                    var byteArray = new Uint8Array(arrayBuffer);
                    Module.ccall('js_LoadFile', 'number', ['string', 'array', 'number', 'number'], [fileName, byteArray, byteArray.byteLength, 1]);
                    Module.updateDiskBox();
                    toastr.success("Disk Image file loaded", fileName);
                } else {
                    toastr.error("An error was encountered loading the disk file", fileName);
                }
            }
        };
        request.send();
        ...
        helprequest.onload = function () {
            if (helprequest.readyState === 4 && helprequest.status === 200) {
                ...
                if (helprequest.responseText !== '') {
                    ...
                    Module.helpTimer = window.setTimeout(function () {
	...
	uploadHighscore: function(userName, scoreId, snapName) {
        ...
        hirequest.onreadystatechange = function () {
            if (hirequest.readyState === 4) {
                ...
                Module.highscoreTimer = window.setTimeout(function () {
                    highScoreButton.innerHTML = defaultText;
                    highScoreButton.className = 'button add';
                }, 3000);
            }
        };
		...
		if (scoreId === '' && snapName !== '') {
			...
			var body = '--' + boundary + '\r\n' + 'Content-Disposition: form-data; name="highscore";' + 'filename="temp.s64"\r\n' + 'Content-type: application/octet-stream\r\n' + 'Content-length: ' + imgData.length + '\r\n\r\n';
