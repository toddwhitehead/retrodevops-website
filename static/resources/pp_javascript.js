...
function c64_checkfile_type(file, startup) {
	...
	if (startup || exthi === 'S64' || exthi === 'CRT') {
		if (exthi !== 'S64') {
            toastr.info("This file type doesn't support auto joystick port detection. Please choose 'Joystick Swap Ports' if needed.", "Joystick Configuraton")
		}
	}
	if (startup || exthi === 'S64' || exthi === 'CRT') {
...
function addControlHandler(parent) {
	const ctrl = {
		...
		cmd_ss: function() {
			Module.snapshotStorage(1);
		},
		cmd_sl: function() {
			Module.snapshotStorage(0);
		},
		cmd_uh: function() {
			Module.uploadHighscore('', '', 'self');
		}
	};
	...
	for (var i = 0; i < children.length; i++) {
		...
		if (child.nodeName === "BUTTON") {
			if (child.name.startsWith('cmd_')) {
				...
				child.addEventListener("touchcancel", function(e) {
					if (e.preventDefault) e.preventDefault();
				}, false);
...
function addButtonHandler(buttonName) {
	$.each(buttonName , function(index, buttonName) { 
		var children = $("[name='" + buttonName + "']");
		...
		if (child.name.startsWith('key_')) {
			child.addEventListener("mousedown", function(e) {
				Module.setKey(this.name.substr(4), 1);
			}, false);
			child.addEventListener("mouseup", function(e) {
				Module.setKey(this.name.substr(4), 0);
			}, false);
			child.addEventListener("touchstart", function(e) {
				Module.setKey(this.name.substr(4), 1);
				if (e.preventDefault) e.preventDefault();
			...
			child.addEventListener("mousedown", function(e) {
				Module.setJoystick(this.name.substr(4), 1);
			}, false);
			child.addEventListener("mouseup", function(e) {
				Module.setJoystick(this.name.substr(4), 0);
			}, false);
			child.addEventListener("touchstart", function(e) {
				Module.setJoystick(this.name.substr(4), 1);
				if (e.preventDefault) e.preventDefault();
...
function addHandler() {
	...
	addButtonHandler(keyboardScreenMap);
	addButtonHandler(joystickScreenMap);
